...
<div class="w-full max-w-screen-xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <a href="{{ route('payslips.index') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-900 transition">
    ...
    <form method="POST" action="{{ route('payslips.store') }}" class="bg-white p-6 rounded-lg shadow-md">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div>
                <label class="block text-sm text-gray-700 font-medium mb-1">Salário Base</label>
                <input type="number" name="base_salary" step="0.01" min="0" placeholder="R$" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" required value="{{ old('base_salary') }}">
            </div>
            <div>
                <label class="block text-sm text-gray-700 font-medium mb-1">Salário Bruto</label>
                <input type="number" name="gross_salary" step="0.01" min="0" placeholder="R$" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" required value="{{ old('gross_salary') }}">
            </div>
            <div>
                <label class="block text-sm text-gray-700 font-medium mb-1">Salário Líquido</label>
                <input type="number" name="net_salary" step="0.01" min="0" placeholder="R$" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" required value="{{ old('net_salary') }}">
            </div>
            <div>
                <label class="block text-sm text-gray-700 font-medium mb-1">Total de Proventos</label>
                <input type="number" name="total_allowances" step="0.01" min="0" placeholder="R$" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" value="{{ old('total_allowances', 0) }}">
            </div>
            <div>
            ...
            <div>
                <label class="block text-sm text-gray-700 font-medium mb-1">Dias Trabalhados</label>
                <input type="number" name="working_days" min="0" max="31" placeholder="Quantidade" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" required value="{{ old('working_days') }}">
            </div>
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-800 bg-gray-50 text-sm" required>
                    ...
                    <option value="generated" @selected(old('status') == 'generated')>Gerado</option>
        ...
        <div class="flex justify-end mt-8 gap-3">
            <a href="{{ route('payslips.index') }}" class="px-4 py-2 rounded bg-gray-100 text-gray-700 hover:bg-gray-200 text-sm border">Cancelar</a>
