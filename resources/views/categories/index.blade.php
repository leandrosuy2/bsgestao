    ...
    <table class="min-w-full text-sm text-left">
        <thead class="bg-gray-100 border-b border-gray-200">
            <tr>
                ...
                <th class="px-4 py-3 font-semibold text-gray-600 uppercase">Descrição</th>
        ...
        <tbody>
            @forelse($categories as $category)
                <tr class="border-b last:border-0 hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3 text-gray-600">{{ $category->code ?? '-' }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $category->description ?? '-' }}</td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('categories.edit', $category) }}" class="inline-flex items-center gap-1 text-white bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 font-semibold px-3 py-1.5 rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536M9 13l6-6m2 2l-6 6m-2 2h6"/>
                                </svg>
                                Editar
                            </a>
                            <form action="{{ route('categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja remover esta categoria?');">
                                @csrf
                                @method('DELETE')
