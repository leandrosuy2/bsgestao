...
class NfeController extends Controller
{
    ...
    public function emitir(Nfe $nfe)
    {
        return redirect()->route('nfe.show', $nfe)->with('success', 'NFe emitida!');
    }

    public function consultar(Nfe $nfe)
    {
        return response()->json(['success' => true, 'message' => 'Consulta OK']);
    }

    public function danfe(Nfe $nfe)
    {
        return response('PDF DANFE')->header('Content-Type', 'application/pdf');
    }

    public function xml(Nfe $nfe)
    {
        return response('XML NFe')->header('Content-Type', 'application/xml');
    }

    public function cancelar(Request $request, Nfe $nfe)
    {
        return redirect()->route('nfe.show', $nfe)->with('success', 'NFe cancelada!');
    }

    public function destroy(Nfe $nfe)
    {
        return redirect()->route('nfe.index')->with('success', 'NFe excluída!');
    }
...
$content | Out-File -FilePath "app\Http\Controllers\NfeController.php" -Encoding ASCII
'@
